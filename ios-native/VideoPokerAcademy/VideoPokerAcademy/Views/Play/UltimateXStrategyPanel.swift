...
private struct HoldRow: View {
    ...
    private var portraitBody: some View {
        HStack(spacing: 4) {
            ...
            Text(String(format: "%.3f", hold.baseEV * avgMultiplier * 2.0))
                .font(.system(size: 10, design: .monospaced))
                .foregroundColor(.white.opacity(0.5))
                .frame(width: 44, alignment: .trailing)

            Text(String(format: "%.3f", hold.eKAwarded))
                .font(.system(size: 10, design: .monospaced))
                .foregroundColor(hold.eKAwarded > 0.005 ? Color(red: 1.0, green: 0.84, blue: 0.0).opacity(0.8) : .white.opacity(0.5))
                .frame(width: 44, alignment: .trailing)
    ...
    private var landscapeBody: some View {
        VStack(alignment: .leading, spacing: 2) {
            ...
            // Line 2: stats
            HStack(spacing: 8) {
                Text(String(format: "EV %.3f", hold.baseEV * avgMultiplier * 2.0))
                    .font(.system(size: 10, design: .monospaced))
                    .foregroundColor(.white.opacity(0.4))
                Text("·")
                    .font(.system(size: 10))
                    .foregroundColor(.white.opacity(0.2))
                Text(String(format: "NH %.3f", hold.eKAwarded))
                    .font(.system(size: 10, design: .monospaced))
                    .foregroundColor(hold.eKAwarded > 0.005 ? Color(red: 1.0, green: 0.84, blue: 0.0).opacity(0.8) : .white.opacity(0.4))
                Text("·")
                    .font(.system(size: 10))
                    .foregroundColor(.white.opacity(0.2))
                Text(String(format: "%.3f", hold.adjustedEV))
