...
struct PayTable: Identifiable, Hashable, Codable {
    ...
    /// Returns pay table rows for WWW mode.
    /// Uses explicit WWW pay table data (with boosted 5-coin payouts and rank-tiered
    /// Five of a Kind) when available, falls back to base rows for unsupported games.
    func wwwRows() -> [PayTableRow] {
        if let explicit = WWWPayTableData.rows(for: id) {
            return explicit
        }
        return rows
    }
