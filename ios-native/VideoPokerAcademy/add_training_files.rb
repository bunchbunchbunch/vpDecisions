...
# Open the project
project_path = 'VideoPokerAcademy.xcodeproj'
project = Xcodeproj::Project.open(project_path)

# Get the main target
target = project.targets.find { |t| t.name == 'VideoPokerAcademy' }
...
# Get the main group (VideoPokerAcademy folder)
main_group = project.main_group.children.find { |g| g.display_name == 'VideoPokerAcademy' }

unless main_group
  puts "ERROR: Could not find VideoPokerAcademy group"
  exit 1
end
...
model_files.each do |file|
  full_path = "VideoPokerAcademy/Models/#{file}"
...
service_files.each do |file|
  full_path = "VideoPokerAcademy/Services/#{file}"
...
viewmodel_files.each do |file|
  full_path = "VideoPokerAcademy/ViewModels/#{file}"
...
# Add TrainingHubView
add_file_to_group(training_group, 'TrainingHubView.swift', target) if File.exist?('VideoPokerAcademy/Views/Training/TrainingHubView.swift')
...
lesson_views.each do |file|
  full_path = "VideoPokerAcademy/Views/Training/Lessons/#{file}"
...
drill_views.each do |file|
  full_path = "VideoPokerAcademy/Views/Training/Drills/#{file}"
...
add_file_to_group(review_group, 'ReviewQueueView.swift', target) if File.exist?('VideoPokerAcademy/Views/Training/Review/ReviewQueueView.swift')
...
lesson_jsons.each do |file|
  full_path = "VideoPokerAcademy/Resources/Lessons/#{file}"
